...
#' @noRd
.sampling_diagnostics_from_fit <- function(fit, backend = "rstan") {
  diag_summ <- .fit_diagnostic_summary(fit, backend)
  summ <- suppressWarnings(.fit_summary(fit, backend = backend))
...
dcvar_diagnostics.dcvar_model_fit <- function(object, ...) {
  .sampling_diagnostics_from_fit(object$fit, backend = object$backend %||% "rstan")
}
