...
but the copula structure below is the common part. The constant model
additionally supports *per-variable* margins, where each variable can use a
different marginal family (see the mixed-margins section below).
...
exposes this by accepting a length-2 `margins` vector. For example, a
roughly-symmetric first series paired with a positively-skewed second series can
be modelled as `normal` and `exponential` margins under a single Gaussian
copula:
...
currently require the Gaussian copula and the constant model.
